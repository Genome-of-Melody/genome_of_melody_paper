...
As of 2024-04-05, this is the most recent version of the file.
"""
import re
...
def clean_volpiano(volpiano, allowed_chars=None, keep_boundaries=False,
                   ...
                   strip_boundaries_at_ends=False):
    ...
    strip_boundaries_at_ends : bool, optional
        If set, will strip hyphens at the beginning and end of the output melody
        after other cleaning steps were applied. Default: False
...
def discard_differentia(volpiano: str, text: str=None, apply_to_repetenda=False) -> str:
    ...
    :param text: The full text of the chant, if given, can help
        ...
        So far, we do not use it.

    :param apply_to_repetenda: If set, will apply the same logic also to single
        barlines ('3' in volpiano). This is applicable especially to responsories
        and responsory verses, but it can also affect hymn section and other structures
        such as introit verses. Check the Volpiano protocols for the full extent
        of what this should apply to.

    """
    barline_position = volpiano.find('4')
    repetendum_barline_position = volpiano.find('3')

    if barline_position < 0 and repetendum_barline_position < 0:
        return volpiano
    if barline_position < 0 and not apply_to_repetenda:
        return volpiano

    if barline_position < 0 and apply_to_repetenda:
        barline_position = repetendum_barline_position
